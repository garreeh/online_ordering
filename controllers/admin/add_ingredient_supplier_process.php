...
if (isset($_POST['add_supplier'])) {
  ...
  // Construct SQL query
  $sql = "INSERT INTO `ingredients_supplier` (supplier_name, address, landline, mobile_number, email, tin)
          VALUES ('$supplier_name', '$address', '$landline', '$mobile_number', '$email', '$tin')";

  // Execute SQL query
  if (mysqli_query($conn, $sql)) {
    // Supplier added successfully
    $response = array('success' => true, 'message' => 'Ingredient Supplier Added successfully!');
    echo json_encode($response);
    exit();
  } else {
    // Error adding supplier
    $response = array('success' => false, 'message' => 'Error Adding Ingredient Supplier!: ' . mysqli_error($conn));
    echo json_encode($response);
    exit();
  }
}
